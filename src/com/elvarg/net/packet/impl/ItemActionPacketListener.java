...
import com.elvarg.world.content.skills.fletching.ArrowshaftData;
import com.elvarg.world.content.skills.fletching.CreateArrowShaftTask;
import com.elvarg.world.content.skills.fletching.FinishedBowData;
import com.elvarg.world.content.skills.fletching.CreateFinishedBowTask;
...
public class ItemActionPacketListener implements PacketListener {
    ...
    private void onItemAction(final Player player, Packet packet) {
        ...
        if (first.getId() == CreateFinishedBowTask.BOWSTRING
                && ItemDefinition.forId(second.getId()).getName().contains("(u)")) {
            final Optional<FinishedBowData> data = FinishedBowData.get(second);
            if (data.isPresent()) {
                CreateFinishedBowTask task = new CreateFinishedBowTask(player, data, 28);
                task.start(player);
            }
            return;
        }
        if (second.getId() == CreateFinishedBowTask.BOWSTRING
                && ItemDefinition.forId(first.getId()).getName().contains("(u)")) {
            final Optional<FinishedBowData> data = FinishedBowData.get(first);
            if (data.isPresent()) {
                CreateFinishedBowTask task = new CreateFinishedBowTask(player, data, 28);
                task.start(player);
            }
        ...
        if ((ItemDefinition.forId(first.getId()).getName().contains("logs")
                && ItemDefinition.forId(second.getId()).getName().equalsIgnoreCase("knife")) ||
                (ItemDefinition.forId(first.getId()).getName().contains("Logs")
                && ItemDefinition.forId(second.getId()).getName().equalsIgnoreCase("knife"))) {
            final Optional<ArrowshaftData> data = ArrowshaftData.get(first);
            if (data.isPresent()) {
        ...
        if ((ItemDefinition.forId(second.getId()).getName().contains("logs")
                && ItemDefinition.forId(first.getId()).getName().equalsIgnoreCase("knife")) ||
                (ItemDefinition.forId(second.getId()).getName().contains("Logs")
                        && ItemDefinition.forId(first.getId()).getName().equalsIgnoreCase("knife"))) {
            final Optional<ArrowshaftData> data = ArrowshaftData.get(second);
            if (data.isPresent()) {
                CreateArrowShaftTask task = new CreateArrowShaftTask(player, data, 28);
                task.start(player);
            }
